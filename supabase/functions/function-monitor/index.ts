...
// ============================================================================
// BeefSynch function-monitor — pings each Edge Function and reports anything
// broken. Runs every 6 hours via cron. Service-role auth only.
//
// For each target:
//   - 200/204 → healthy
//   - 400/401/403 → alive but rejecting (which is healthy for protected fns)
//   - 5xx OR network error/timeout → broken; counted as a failure
// Results logged to edge_function_health_log. If any failures, an alert email
// goes out via Resend.
// ============================================================================

interface Target {
  name: string;
  /** Acceptable status codes that signal "alive". */
  okStatus: number[];
  /** Optional: send a token so JWT-protected functions return 401 instead of 500. */
  expectsAuth: boolean;
}

const TARGETS: Target[] = [
  // verify_jwt:false — should accept service-role bearer; we'll send service role
  ...
  // verify_jwt:true — without a real user JWT they should 401, which is healthy
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Skip self-probing if invoked from the body marker (avoids a probe loop).
    let body: any = null;
